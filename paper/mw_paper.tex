...
Peru's labor market is characterized by high informality: approximately 72--75\% of employed workers are informal by INEI's comprehensive definition. The MW directly affects roughly 10--15\% of the workforce classified as formal dependent workers under INEI's formality variable (\texttt{ocupinf == 2}), which requires social security registration or a written employment contract.
...
This estimator does not require parallel trends and is valid for all three events.
...
\begin{tabular}{lccc}
\toprule
Bin width & $R$ (Event A) & $R$ (Event B) & $R$ (Event C) \\
\midrule
S/25 (canonical) & 0.697 & 0.829 & 0.829 \\
S/50             & 0.677 & 0.206$^{\dagger}$ & 0.662 \\
S/100            & 0.477 & 0.754 & 0.613 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item \textit{Note}: Canonical inv-abs background correction; excess window S/250; all three events; formal dependent workers. Coarser bins reduce the number of resolution points in the missing and excess zones, introducing noise.
\item[$\dagger$] At S/50, $\text{MW}_\text{new} = 930$ does not fall on a bin boundary (bins span [900,950), [950,1000), \ldots). Workers bunching at S/930--S/949 are captured in bin [900,950) (center = 925), which lies below $\text{MW}_\text{new}$ and is therefore classified as part of the \textit{missing} zone rather than the excess zone. This boundary misclassification severely depresses $R$ for Event B and should not be interpreted as a genuine robustness failure. The S/25 canonical grid is preferred because it minimises boundary misclassification for all three MW thresholds.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\textbf{Employment effects cannot be identified.} The event study fails pre-trends for Events B ($p = 0.007$) and C ($p = 0.017$); Event A has no testable pre-period. The IV approach fails due to weak first stages ($F < 3$ in all events). The bunching ratios (0.70--0.83, bootstrap CIs [0.57--1.02]) are consistent with both modest displacement (up to 28\% of affected workers) and full redistribution. The data cannot distinguish between these scenarios.

\textbf{What can be concluded}: Peru's MW increases produce a clear, placebo-verified redistribution of the formal wage distribution. Workers near the MW floor receive raises---the pre-post estimator documents 6.78--13.02 percentage points of missing mass below the old floor across three events, with 70--83\% reappearing as excess mass at the new floor. The intensive margin (hours) shows no adjustment. Compression of 3--7 percentage points is present but 41--92\% is mechanical composition. The residual genuine compression ($-0.005$ to $-0.029$ log points) represents modest spillover above the MW threshold; with individual controls, the compression coefficient is $-0.045$ ($p < 0.001$).
