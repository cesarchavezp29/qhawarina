...
For Event B, approximately 41\% of the $-0.050$ DiD reflects the composition effect---workers pushed from below the MW to the new floor entering the compression zone and diluting its mean log wage. The remaining 59\% ($-0.029$ log points) represents genuine relative wage compression. For Events A and C, the composition effect dominates (78\% and 92\%). The compression DiD is a descriptive statistic of distributional change, not a causal estimate.\footnote{The mechanical decomposition assumes that new entrants' wages are determined independently of the MW---i.e., they enter at the MW floor because it is the market wage in their sector, not because the MW creates a mass point. If MW increases drive lighthouse-effect formalization, new entrants' wages \textit{are} set by the MW, and classifying their compositional contribution as ``mechanical'' understates genuine compression. Our mechanical share is therefore an upper bound on the fraction of compression that is composition-driven.} The distributional patterns we document are consistent with \citet{engbom2022}, who find that Brazil's national MW compresses the wage distribution from below, and with \citet{jales2018}, who uses a density discontinuity approach to estimate MW effects in Brazil.
