...
We compare these to the average IRP on all non-event days (days outside
any $\pm$5-day window).  Abnormal IRP is the difference between the
$\pm$3-day window mean and the non-event mean.  A one-sided Welch
$t$-test tests whether event-window IRP ($\pm$5 days) exceeds the
non-event mean.
...
$+10.5$), confirming that the post-filter did boost that event meaningfully;
we note this explicitly as a limitation.  For the remaining six non-floored
events, pre- and post-filter results are directionally consistent.
...
\begin{figure}[htbp]
  ...
  \caption{Daily IRP and IRE, January 2025 -- March 2026. Gray bars show daily raw values; colored lines show the EMA-smoothed series ($\alpha = 0.30$). Dashed vertical lines mark major political and economic events. The horizontal reference line at IRP = 150 indicates the elevated-risk threshold.}
...
pipeline expansion.  The residual 9\% divergence reflects genuine
...
\begin{figure}[htbp]
  ...
  \caption{Monthly IRP versus monthly IRE, January 2025 -- March 2026. Each point is a calendar month. The line shows the OLS fit with 95\% confidence band. With $N = 15$ monthly observations, the regression is underpowered and the band is wide.}
