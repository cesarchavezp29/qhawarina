...
the same FWL COVID treatment gives a peak GDP response of $-0.179$\,pp,
closely matching the baseline $-0.195$\,pp. The 90\% bootstrap confidence
interval from VAR(2) is $[-0.71,\,+0.30]$\,pp, slightly wider than the
baseline owing to the additional parameters but qualitatively identical.
Lag order uncertainty does not alter the substantive conclusion.
...
response of $-0.187$\,pp per 100\,bp. Restricting to a four-variable
system that excludes ToT gives $-0.201$\,pp. Re-estimating with the level
of the reference rate instead of first differences gives $-0.226$\,pp; the
ADF test for the level series confirms near-unit-root behaviour, but
estimation in levels remains valid under \citet{sims1990}. Across all
ordering variants, peak GDP responses lie in $[-0.13,\,-0.29]$\,pp, each
...
Construction A yields $F=2.1$ and Construction B yields $F=18.4$;
...
GDP--poverty coefficient, mixing frequencies. As a consistency check, we
estimate a 3-variable annual VAR(1) with [GDP, CPI, Rate], $T=19$
(excluding 2020), Cholesky ordered GDP $\to$ CPI $\to$ Rate. The annual
peak GDP response to a 100\,bp rate shock is $-0.51$\,pp at $h=1$ year.
Multiplying by $\hat\beta=-0.656$ gives a chained annual effect of
$+0.33$\,pp on poverty---within the range implied by the quarterly chain.
To propagate uncertainty jointly, we draw 2,000 bootstrap VAR replicates,
extract the peak GDP response at $h=3$ quarters, and draw $\hat\beta$ from
